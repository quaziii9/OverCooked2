...
public class Player2InteractController : MonoBehaviour
{
    ...
    public bool IsHolding { get; set; } // 현재 오브젝트를 잡고 있는지 확인하는 변수
    public bool CanActive { get; set; } // 재료를 제외한 모든 오브젝트 활성화 확인값
    ...
    [FormerlySerializedAs("PlayerInputSystem")] [Header("PlayerInputSystem")] [SerializeField]
    ...
    private void HandleObject(GameObject obj, bool isPickingUp = true)
    {
        ...
        else
        {
            ...
            {
                _placeTransform = interactObject.transform.parent.GetChild(1).localPosition + new Vector3(0.072f, 0.006f, 0.024f);
            }
            else if (interactObject.transform.parent.CompareTag("WizardCounter"))
            {
                _placeTransform = interactObject.transform.parent.GetChild(1).localPosition + new Vector3(0.10746f, 0.00500000005f, 0.0235699993f);
                Debug.Log(interactObject.transform.parent.GetChild(1).name);
            }
            else if (interactObject.transform.parent.CompareTag("MineBoard"))
            {
                Debug.Log("MineBoard");
                _placeTransform = interactObject.transform.parent.GetChild(1).localPosition + new Vector3(0f, 0.0055f, 0f);
            }
            else
            {
