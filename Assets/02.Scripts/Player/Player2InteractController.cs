...
public class Player2InteractController : MonoBehaviour
{
    ...
    void StartCuttingProcess()
    {
        ...
        var ingredient = cuttingBoard.transform.parent.parent.GetChild(2).GetChild(0).GetChild(0)
            .GetComponent<Ingredient>();

        if (cuttingBoard.CoTimer == null &&
            ingredient.currentState != Ingredient.IngredientState.Chopped) // 한번도 실행 안된거면 시작 가능
        {
    ...
    private void TablePlaceOrDropObject(bool drop)
    {
        ...
        if (drop)
        {
            ...
            if (objectHighlight.transform.parent.GetChild(2))
            {
                ...
                if (potAndPan.name.Equals("PFX_PanFire"))
